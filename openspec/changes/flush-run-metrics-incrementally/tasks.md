...
- [x] 11.1 Offline verification: `npm run typecheck`, `npm run build`, and `npm run lint:md` pass cleanly; the new and directly-affected test suites pass. The full `npm test` run additionally reports 10-11 pre-existing failures (Windows path/chmod semantics and timing-sensitive tests under parallel load) that are not fixed by or related to this change — confirmed by running the identical suite against `main` with none of this branch's changes applied and observing the same failures.
