...
Live runs showed a stage can finish "done" with all gates green having only planned the work (one header edit, ERC "clean" on an empty sheet), after which every later stage runs against a design that is not there. Stage completion is therefore judged by repo state and the applicable deterministic gate: the schematic stage requires at least one symbol plus drift-clean BOM/PINOUT; layout-draft requires a board containing a footprint, the LAYOUT.md marker, and a fresh clean KiCad DRC result; outputs requires the complete non-empty artifact set produced by `exportFab` plus the ordering BOM, while also rechecking source-board DRC. A receipt is written only after every exporter job succeeds; it binds the current board, schematic, BOM source, KiCad `.gbrjob` file list, and every concrete output by SHA-256. Resume recalculates those hashes, and agent file tools refuse receipt edits. The marker that `init` scaffolds, a lone Gerber, or a receipt without matching files and sources proves nothing. `runCreate` re-checks the contract after each successful run and halts with a resume hint rather than advancing. Alternative considered: prompting harder; rejected because a contract that is not enforced is advisory.
