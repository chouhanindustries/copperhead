...
- **New CLI surface**: `copperhead draft` and `copperhead score`, both LLM-free and network-free; `check` gains legibility advisories and a `legibility` object (findings, counts, skipped, disabled, score) in `--json`.
