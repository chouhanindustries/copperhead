...
- **New deterministic checker `src/kicad/legibility.ts`**, read-only, built on the existing `src/kicad/sexp.ts` parser and never serializing (SPEC §1.3, design D4). It reports one finding per defect with coordinates and an actionable fix, over these check families: symbol-body overlap; text collision (refdes, value, or label text over a symbol body, a wire, or other text); off-grid symbol origins, wire endpoints, and label positions; label orientation rotated where horizontal fits; content bounding box versus page size (utilization too low, or content outside the drawing frame); nearest-neighbour crowding below a readable pitch; ungrouped symbols and missing or unlabeled group boxes; overlapping group boxes; wires crossing a symbol body without terminating on a pin; empty title-block fields.
...
- **Config**: `.copperhead/config.json` gains an optional `legibility` block (thresholds, per-check severity overrides, disable flag).
