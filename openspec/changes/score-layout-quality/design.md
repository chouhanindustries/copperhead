...
Net identity spans two file generations. KiCad 8/9 writes `(net 3 "GND")` on pads, bare `(net 3)` on segments and zones, and a top-level `(net 3 "GND")` table; KiCad 10 writes `(net "GND")` everywhere and keeps the table for net 0 alone. `src/kicad/pcb.ts` resolves all three forms to a name, so the same code reads the 2024 fixture and the 2026 demo. Because the KiCad 10 table is empty, the board's net-name set is built as the union of the table and every net resolved on a pad, segment, via, or zone: constraint text is matched against that set, and a set built from the table alone would silently degrade every net-named hard row to n/a on a KiCad 10 board.
...
**Hard vs soft.** Hard rows exist only when a matching key exists in `.copperhead/constraints.json`, matched by key *suffix* so that a project's own namespacing (`mech.load_trace_width_mm`, `pwr.trace_width_mm`) lands on the same measurement. A board with no constraints gets an empty hard table and a soft-only score, and every hard row names the constraint key it came from, which is what makes "every hard metric traces to a key" checkable rather than aspirational. A row whose measurement the board cannot answer (no mounting holes, no bypass caps, no tracks on a power net) is `n/a` with a note, never a silent pass. What an `n/a` row does to the score depends on whether anything is placed. On the empty scaffold, `n/a` rows are excluded from the hard term: nothing is placed yet, `n/a` is the honest default, and the placement and routing terms already hold the score down. On a board with footprints, any `n/a` row forfeits the hard term entirely. Excluding `n/a` rows there let deletion beat misplacement: deleting the bypass caps turned the failing decoupling row into an `n/a` row, shrank the denominator, and sprang the hard term back to full marks, so a board with no decoupling at all outscored a board with badly placed decoupling. Forfeiting the term means an `n/a` row can never shrink the denominator and can never score above the same row failing, so a fail turning into `n/a`, deletion of the policed component included, can never improve the 20-point hard term. The guarantee is scoped to that term: the soft terms are measured on whatever remains on the board, so deletion still moves routing, density, and directness on their own merits.
...
The `/usr/share/kicad/demos` corpus is a second tier: the test skips when the directory is absent rather than failing, because the demos are a KiCad install artifact and not a Copperhead dependency. CI installs `kicad-demos` explicitly (the package is only Recommended by `kicad`, so `--no-install-recommends` would otherwise drop it), which means the corpus tier, including the labeled routed/unrouted pair, runs on every CI job as well as on developer machines; the skip path exists for machines without the demos. When the corpus runs it asserts the labeled pair (routed > unrouted) and that all boards score without throwing. `npm run layout:corpus` writes the baseline distribution to `test/fixtures/layout-corpus-baseline.json`, committed and timestamp-free so that a scorer change shows up as a reviewable diff rather than as nothing at all.
...
- **Two demo boards are ~70 MB.** Parsing them peaks around 1 GB of heap and the full corpus sweep takes about 19 s. It is guarded by an explicit per-test timeout, and it runs in CI (the workflow installs `kicad-demos`), so that cost lands on every CI job; a machine without the demos still skips it.
