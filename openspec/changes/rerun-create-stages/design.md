...
**The record is probe-gated at commit time.** A run can pass the loop's gates without producing the stage's artifacts (nothing in `finish` requires the products to exist — the #23 failure mode). The record asserts "this stage's work exists as committed", so `beforeCommit` writes it only if the stage's completion probe passes against the post-run state; otherwise the commit still lands (partial verified work is kept) but the stage stays unrecorded, is classified incomplete next time, and re-runs. The runner prints when a record is withheld. This is also what makes composition with #29's post-run contract check safe: a contract-failing stage never gains a record, so "re-run to continue" remains true.
...
**A record never outranks a failing probe.** For recorded stages, classification re-runs the probe first: if it fails now, the stage is `incomplete` — its work product was deleted after completion, or the probes have grown stricter since the record was written (e.g. after upgrading past #29). Without this, a record would substitute "ran and committed" for "work exists", and a stage whose artifact disappeared would be skipped forever while its consumers went stale against a missing input.
...
- [PR #29 touches the same stage table in `src/commands/create.ts`] → semantic composition is safe by construction: probe bodies are untouched, stricter probes drop into both the unrecorded fallback and the record gate, and probe-gated records mean #29's "stop when the contract is unmet" cannot deadlock against a pre-committed record. Textual conflicts are expected and accepted — #29 rewrites the module docstring and anchors its post-run check on the old sequential runner, so whichever PR lands second rebases the runner hunks and #29's mocked-loop test by hand.
