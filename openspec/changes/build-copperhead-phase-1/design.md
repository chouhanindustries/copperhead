...
Because completion is inferred from files, `create` requires a clean tree at command entry before evaluating any stage marker, except that the resolved `--brief` file may be the sole uncommitted path: it is pipeline input rather than a completion marker. This separates intentional input/intra-pipeline dirtiness (the brief and OpenSpec bootstrap state passed to the first stage with `allowDirty`) from stale partial output left by `--keep-on-fail`. A kept stage failure tells the user to recover before rerunning; otherwise a partial `outputs/` or `firmware/` directory could incorrectly skip an unverified stage. If the first ordinary stage failure/refusal occurs before any stage commits, `create` also restores the command-entry snapshot so OpenSpec bootstrap files do not make its own next-run preflight fail, then recreates an initially untracked in-repo brief because `git stash create` does not capture untracked files.
