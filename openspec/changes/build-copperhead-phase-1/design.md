...
Because completion is inferred from files, `create` classifies command-entry dirt before evaluating stage markers. Foreign user paths are refused. Recognized Copperhead-managed stage artifacts may resume, but each must pass its stage-specific completion gate before the resume path can commit it. The resolved `--brief` may also be uncommitted because it is pipeline input rather than a completion marker. A kept stage failure writes an ignored refusal marker; while it exists, every dirty path other than the brief is treated as known-unverified and `create` stops before `isComplete`, so a partial `outputs/` or `firmware/` directory cannot skip its failed stage. Once manual recovery leaves only the brief dirty, the stale marker is cleared. If the first ordinary stage failure/refusal occurs before any stage commits, `create` also restores the command-entry snapshot so OpenSpec bootstrap files do not make its own next-run preflight fail, then recreates an initially untracked in-repo brief because `git stash create` does not capture untracked files.
