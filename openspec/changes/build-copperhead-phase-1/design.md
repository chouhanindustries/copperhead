...
Because completion is inferred from files, `create` classifies command-entry dirt before evaluating stage markers. Foreign user paths are refused. Recognized Copperhead-managed stage artifacts may resume, but each must pass its stage-specific completion gate before the resume path can commit it. The resolved `--brief` may also be uncommitted because it is pipeline input rather than a completion marker, and the guarded resumed-stage commit explicitly excludes that path even when it lives under a managed directory. A loop-level success does not override the outer stage contract: when `isComplete` still fails, default mode restores the last completed pipeline state; `--keep-on-fail` rewinds the loop's commit while retaining its staged files and writes the ignored refusal marker. While that marker exists, every dirty path other than the brief is treated as known-unverified and `create` stops before `isComplete`, so a partial `outputs/` or `firmware/` directory cannot skip its failed stage. Once manual recovery leaves only the brief dirty, the stale marker is cleared. If the first ordinary stage failure/refusal occurs before any stage commits, `create` also restores the command-entry snapshot so OpenSpec bootstrap files do not make its own next-run preflight fail, then recreates an initially untracked in-repo brief because `git stash create` does not capture untracked files.
