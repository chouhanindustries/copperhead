...
- Implement safety rails: repo-root path sandboxing, dirty-tree refusal (including clean entry for `create` before resume-marker checks, except that its resolved brief input may be the sole uncommitted path), snapshot/rollback on failure, an explicit `--keep-on-fail` debugging escape hatch that skips cleanup after unrecoverable failure without permitting success or commit, shell-safe audit-preserving manual recovery, secret redaction in transcripts, `UNVERIFIED` flagging of invented parts.
