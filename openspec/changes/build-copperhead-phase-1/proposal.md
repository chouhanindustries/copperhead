...
- Implement safety rails: repo-root path sandboxing, dirty-tree refusal (`create` rejects foreign entry changes, gate-validates managed resume artifacts, accepts its resolved brief as uncommitted input, and blocks all retained failed-stage dirt), snapshot/rollback on failure, an explicit `--keep-on-fail` debugging escape hatch that skips cleanup after unrecoverable failure without permitting success or commit, shell-safe audit-preserving manual recovery, secret redaction in transcripts, `UNVERIFIED` flagging of invented parts.
