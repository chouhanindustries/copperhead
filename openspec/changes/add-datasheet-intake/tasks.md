...
- [ ] 7.3 Run the end-to-end demo acceptance: GT-1, GT-2, GT-3 back to back, cold, twice, each writing a registry entry and downloadable manifest, inside 3 minutes; record the fallback demo video
  - Acceptance PASSED 2026-07-26, twice from a cold server (`.next` and registry wiped): scripts/demo-acceptance.mjs, 32/32 checks per pass, arcs 2.3s/0.1s and 2.6s/0.2s against the 180s limit. GT-1 and GT-2 reach their REFUSE via the correction flow (real-part extractions hold by design), GT-3 holds naming the field. Remaining for the tick: record the fallback demo video.
