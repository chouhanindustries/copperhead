...
It SHALL search every `.kicad_sym` library found in the symbol search directories (the `KICAD*_SYMBOL_DIR` overrides, then the standard install locations) and return matching identifiers as `Lib:Name` lib_ids, directly usable as a schematic `lib_id`. Matching SHALL be case-insensitive and separator-insensitive (`_`, `-`, `.` ignored), so `Rotary_Encoder` finds `RotaryEncoder_Switch`. Candidates from all libraries SHALL be ranked together — exact matches, then prefix, then substring — before any result cap applies, so a strong match in a late-scanned library is never displaced by weak matches from earlier ones. Sub-unit child symbols (`Name_<unit>_<style>`) are internal structure and SHALL NOT be returned.
