...
- [x] 7.4 Replace `options: Record<string, unknown>` with a local typed `QueryOptions` interface so option names are compile-checked without making `tsc` depend on the optional SDK being present after `npm install --omit=optional`
...
- [x] 7.8 Harden against ambient SDK configuration: pass `settingSources: []`, strict empty MCP config, empty skills/plugins, `dontAsk`, and auto-memory off; disable SDK session persistence unless explicit session resume is enabled; strip direct/custom/cloud billed-auth routes while retaining `CLAUDE_CODE_OAUTH_TOKEN`; cover every option offline and align SPEC/docs/AGENTS/CLAUDE guidance
