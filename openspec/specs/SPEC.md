...
- Everything written under `.copperhead/runs/` redacts anything matching `sk-[A-Za-z0-9_-]+` at write time: the structured transcript, the verbatim provider log (`raw.log`), the console mirror (`console.log`), and the summary. `raw.log` records request and response payloads exactly as they crossed the wire, so its redaction is load-bearing rather than incidental
...
- **AC-8.10 (redaction holds)** A string matching `sk-[A-Za-z0-9_-]+` planted in any metadata field appears redacted in both the persisted `run-start` event and `summary.md` (extends AC-4.1 to the new surfaces), and in `raw.log` and `console.log`.
- **AC-8.11 (turns stream)** With a provider that supports it, assistant text reaches the terminal while the turn is still being generated, not in one block when it ends, and appears exactly once (the completed turn is not reprinted). A provider or endpoint that cannot stream produces the same output at the end of the turn, and an endpoint that refuses a streamed request is retried once without streaming — recorded in `raw.log`, not surfaced as a run failure. Token accounting is identical on both paths.
- **AC-8.12 (verbatim traffic)** Every run writes `raw.log` beside its transcript, one JSON object per line, live: the request and response payloads for the API providers, and the CLI's own messages for the saved-login providers. A payload that cannot be serialized, or one large enough to threaten the disk, is truncated or noted rather than dropped silently or raised as an error.
- **AC-8.13 (console mirror)** Every run writes `console.log` beside its transcript, live, containing every durable line the run printed with ANSI stripped; the transient status line and in-place redraws are not mirrored.
