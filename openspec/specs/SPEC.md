...
### AC-16 · Incremental run metrics & artifact commits (change: flush-run-metrics-incrementally, issue #138)

- **AC-16.1 (per-call events)** An `llm-call` event is appended to `transcript.jsonl` immediately after every provider call resolves or rejects, before the next turn's call begins, carrying `turn`, `stage`, `callId`, `model`, `provider`, `tokensIn`/`tokensOut`, `cacheRead`/`cacheWrite`, `cacheHit`, `latencyMs`, `startedAt`/`finishedAt`, `stopReason`, `toolCalls`, and `error` (`null` on success). Written durably (fsync'd), not through the transcript's ordinary buffered append.
- **AC-16.2 (SIGKILL survival)** `SIGKILL`-ing a run mid-stage, after N calls have completed and one is in flight, leaves `llm-call` events for all N completed calls on disk.
- **AC-16.3 (live metrics)** `.copperhead/runs/<runId>/metrics.json` exists after the first call and stays current (its last-update timestamp advances) for the whole duration of the run, including during a single call that runs long — via the loop's existing liveness heartbeat, not only between calls. Its `status` is `'running'` for the entire healthy duration of the run, never a placeholder for "not finished yet"; it is `'stalled'` only when the loop's own stall detector actually fires.
- **AC-16.4 (stage-boundary reports)** `.copperhead/runs/REPORT.md`/`report.json` are regenerated when a `create` stage starts (showing that stage as `running`) and again when its outcome is known — not only on stage failure or full-pipeline completion as before this change.
- **AC-16.5 (artifact commits, never amended)** When `commitRunArtifacts` is true (default), every terminal path of a run (success, refusal, dry-run, unattended failure) makes a new, standalone commit containing exactly that run's `transcript.jsonl`, `metrics.json`, and `summary.md`, added by exact path — never a directory, never `git add -A`, and never an amend of an existing commit (a run's own commit may already be followed by a separate openspec-archive commit). `create` separately commits the regenerated `REPORT.md`/`report.json` when a stage's outcome is known (success, failure, or stop) and at full-pipeline completion — not at stage start, which runs before that stage's own `gitPreflight` has validated the repo even once, and must stay a disk write only.
- **AC-16.6 (abnormal exit still leaves data committed)** A run ending in `provider-error` or `turn-timeout` still produces an artifact commit once its rollback completes; with `commitRunArtifacts: false`, the files are still written to disk and the run's log output names their path as uncommitted.
- **AC-16.7 (totals reconcile)** For a clean run, the sum of `tokensIn`/`tokensOut` across all `llm-call` events equals the totals in that run's `run-end` event.
- **AC-16.8 (cache provenance reconciles)** The count of `llm-call` events with `cacheHit: true` equals the `cacheHits` value `report.json` reports for that stage.
- **AC-16.9 (interrupt handling, no rollback)** `SIGINT`/`SIGTERM` are handled by a synchronous handler prepended ahead of any other process-level signal listener; the working tree is left exactly as the interrupted turn left it (no rollback — a deliberate deviation from the unattended-failure path), a final `metrics.json` snapshot is written, and (when `commitRunArtifacts` is true) a commit of only the run-artifact files is made before the process exits with code 130.
