...
Recover before rerunning. `do` refuses the dirty tree unless you explicitly choose `--allow-dirty`; `create` requires a clean tree at pipeline entry except that its resolved `--brief` input may be the only uncommitted path. Copperhead preserves that input across first-stage rollback. The stricter `create` preflight still rejects partial files such as `firmware/`, `outputs/`, `.copperhead/constraints.json`, or a validated `openspec/changes/<id>` directory because they can otherwise look like completed/trusted pipeline state. `--dry-run` and `--keep-on-fail` are mutually exclusive.
