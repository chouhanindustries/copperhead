...
**The sheet is aligned and balanced, not merely legal.** A final pass makes the drawing look deliberate: symbols in a column share one axis, siblings (a decoupling row, the ground symbols under a group) sit at uniform spacing and uniform height, a chain of passives between two aligned pins is placed collinearly so its wires run dead straight, paired structures like crystal load caps mirror each other, each group's contents are centered in their box, and the groups are distributed so the page reads balanced instead of packed into a corner. None of this is left to chance: symmetry and alignment are computed properties, and the score measures them (axis alignment, spacing uniformity, straight-wire ratio, whitespace balance, pair symmetry) so an update that makes sheets uglier fails its benchmark even when every hard rule still passes.
