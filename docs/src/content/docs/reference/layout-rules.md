...
- **Populated board (any footprints):** any `n/a` row forfeits the entire 20-point hard term. An unanswerable row can never shrink the denominator and can never score above the same row failing, so a fail turning into `n/a`, including through deleting the component a constraint polices, never improves this term. The guarantee is about the hard term only: soft terms are measured on whatever remains on the board, so they can still move. On the reference board this rule is what stops deleting the bypass capacitors from outscoring misplacing them.
