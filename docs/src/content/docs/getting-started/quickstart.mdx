...
2. **Choose a model backend.**
   ...
   To run entirely on your own machine with no cloud account, load a **tool-capable** model in [LM Studio](https://lmstudio.ai/), start its server, and:

   ```bash
   export COPPERHEAD_MODEL=lmstudio
   # optional: point elsewhere, e.g. Ollama or vLLM
   # export LMSTUDIO_BASE_URL=http://localhost:1234/v1
   ```

   Nothing leaves your machine and no key is involved. The model must support function/tool calling. See [Local models (LM Studio)](/reference/configuration/#local-models-lm-studio).
