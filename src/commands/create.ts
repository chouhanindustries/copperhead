...
export const STAGES: Stage[] = [
  ...
  },
  {
    ...
    prompt: () =>
      'Stage 5: first-draft layout. Use populate_board to import installed footprints at planned coordinates before routing; never invent pads from library names. Use search_footprints to discover installed IDs, then verify package compatibility. An unavailable footprint requires a verified replacement in the BOM and schematic intent before retrying placement. Rule-driven placement written as real coordinates: connectors on edges, decoupling at IC pins, ESD at connectors, keepouts honored. Route power and short critical nets; leave the rest as ratsnest. Every routed net must pass run_drc. Then write the "## Draft quality" section in LAYOUT.md: exactly what is fine and what a human or specialist tool should redo. Non-optimal is acceptable; unlabeled non-optimal is not.',
  },
  {
    name: 'outputs',
    isComplete: async (root) => {
