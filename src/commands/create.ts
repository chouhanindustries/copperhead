  ...
  while (queue.length) {
    ...
    for (let attempt = 1; ; attempt++) {
      ...
      if (name === 'schematic' && attempt > 1) {
        ...
        if (rescaffolded) opts.log(`stage schematic: re-scaffolded empty KiCad project after rollback, wired into config`);
      }
      opts.log(
        `stage ${name}: running${rerunNote}${attempt > 1 ? ` (attempt ${attempt}/${config.maxStageRetries + 1})` : ''}`,
      );
      ...
      if (attempt > config.maxStageRetries) {
        opts.log(
          `stage ${name}: ${failure}; exhausted ${config.maxStageRetries} auto-retry(ies). Stopping for a human.`,
        );
        break;
      }

      opts.log(`stage ${name}: ${failure}; asking the model whether to retry…`);
      ...
      opts.log(`stage ${name}: diagnosis → ${diagnosis.verdict} — ${diagnosis.reason}`);
      if (diagnosis.verdict === 'abort') {
        opts.log(`stage ${name}: recovery supervisor recommends stopping for a human.`);
        break;
      }
