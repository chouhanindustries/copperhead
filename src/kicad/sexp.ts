...
const namesPowerNet = (libId: string, powerSyms: Set<string>): boolean =>
  isPowerSymbol(libId, powerSyms) && libId !== 'power:PWR_FLAG';
...
export async function listNets(rootSch: string): Promise<string[]> {
  ...
  for (const sheet of sheets) {
    ...
    for (const { sym } of symbolsOf(sheet)) {
      if (namesPowerNet(sym.libId, powerSyms)) names.add(sym.value);
    }
  }
  return [...names].sort();
}
  ...
  for (const sheet of sheets) {
    ...
    for (const { sym, mirror } of symbolsOf(sheet)) {
      ...
      for (const pin of defs) {
        ...
        uf.find(k);
        if (namesPowerNet(sym.libId, powerSyms)) {
          netNameAt.set(k, sym.value);
        } else if (!isPowerSymbol(sym.libId, powerSyms)) {
