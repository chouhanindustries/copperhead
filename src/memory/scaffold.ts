...
- \`maxEditBytes\`: largest single \`edit_file\` payload accepted for a KiCad file (default 8192; \`0\` disables). Keeps a stage working one part at a time instead of emitting a block that must land whole or be lost whole. Docs are never capped.
- \`maxUnverifiedEdits\`: KiCad edits allowed per file kind before the matching check must run (default 1; \`0\` disables). \`run_erc\` clears the schematic count, \`run_drc\` the board count.
- \`checkpointCommits\`: commit the run's touched paths whenever ERC/DRC goes clean after an edit, and roll back to that point instead of to the start of the run (default true)
- \`stageBudgets\`: per-create-stage spend limits, e.g. \`{"schematic": {"maxTokensOut": 120000, "maxWallMs": 2400000, "maxTurnOut": 8000}}\`. Output-token and wall limits end the stage; the per-turn limit asks the model to split the unit and keeps going.
