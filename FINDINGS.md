...
## F-8 DEFECT: ERC/DRC `ok` ignores severity, so warning-level report entries block stage completion

- **Where:** `src/kicad/report.ts` `normalizeReport` (`ok: violations.length === 0` over every folded entry regardless of its `severity` field). Also identified independently in PR #72 (credited per the recurrence convention); reproduced and fixed here from the code with unit coverage.
- **Symptom:** kicad-cli ERC/DRC JSON reports carry `severity: "warning"` entries (advisory checks, exclusions) alongside errors. Any such entry made `report.ok` false, so a schematic or board that is error-clean but carries one advisory warning can never satisfy a stage completion gate or the agent loop's finish gate.
- **Suggested (P1, fixed in this PR):** gate `ok` on non-warning entries only; entries with no severity field remain blocking for safety; warnings stay in the violations list so they are still surfaced. Covered by `test/report-severity.test.ts` (warning-only ok, error-among-warnings blocks, missing severity blocks, empty clean).
- **Status:** fixed in this PR.

## F-9 BLOCKER: no tool exposes installed-library pins, so a part whose exact lib_id is absent cannot be authored faithfully and the stage refuses

- **Where:** the agent tool set (`src/agent/tools.ts`) vs the stage 4 prompt's hard rule "never invent pin numbers, names, or electrical types" (`src/commands/create.ts`).
- **Symptom:** the stage gives the model `verify_symbols` (cross-checks symbols already authored) and `read_file` (cannot reach the system `.kicad_sym` libraries), but nothing that returns a real part's pins before authoring. For a part whose guessed lib_id is not the installed name (the brief needs `ESP32-C3-MINI-1`; the installed RF_Module library has `ESP32-C3-DevKitM-1`, `ESP32-C3-WROOM-02`, not that exact symbol), the model correctly refuses rather than invent a ~53-pad footprint from memory. Across two runs, stage 4 refused three times for exactly this reason (run summaries at `.copperhead/runs/2026-07-24T10-05-50-017Z` and the run3 log: "verify_symbols returns not found for every vendor symbol ... reproducing from memory would require inventing pin numbers"). The integrity gate is right; the missing capability is the bug.
- **Suggested (P1, fixed in this PR):** add a read-only `lookup_symbol` tool that resolves a lib_id against the installed libraries and returns the real pin table (number, name, electrical type), or the closest real symbol names when the exact one is absent. Point the stage 4 prompt at it: look up before authoring, and adopt a suggested real name rather than inventing a missing one. Tool built on the existing `resolveLibrarySymbol`; covered by `test/lookup-symbol.test.ts`.
- **Status:** fixed in this PR. Combined with F-4 (preflight now flags a missing library set), this closes the loop that made stage 4 unreachable in a stock-library-absent environment.

## F-10 INEFFICIENCY: the default 40-turn stage budget cannot fit a faithful medium-board schematic capture

- **Where:** `maxTurns` default (`src/config.ts`) vs the stage 4 workflow the prompt mandates (one part at a time, lookup, edit, ERC per part or batch).
